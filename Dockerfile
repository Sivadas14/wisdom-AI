...
# Uses printf+pipe so the Python spans multiple lines without Dockerfile parse errors.
# Any startup crash (missing module, bad attribute, etc.) fails the BUILD here,
# giving a clear error rather than a silent container crash at runtime.
RUN printf '%s\n' \
    'import sys, os' \
    'sys.path.insert(0, ".")' \
    'os.environ.setdefault("ASAM_DB_URL", "postgresql+asyncpg://x:x@localhost/x")' \
    'os.environ.setdefault("ASAM_JWT_SECRET", "smoke-test")' \
